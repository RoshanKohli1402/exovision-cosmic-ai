                ...
                <div className="mt-6 p-4 bg-primary/10 rounded-lg">
                  <div className="flex items-center justify-between">
                    ...
                    <CosmicButton variant="primary" onClick={processFile} disabled={isProcessing} className="hover:scale-105 flex-shrink-0">
                      {isProcessing ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Processing...</>) : (<><Zap className="w-4 h-4 mr-2" />Run Analysis</>)}
                    </CosmicButton>
                  </div>
                </div>
              )}
            ...
            <CosmicCard className="p-8 h-full">
              ...
              {results && (
                <div className="space-y-6">
                  <div className={`p-4 rounded-lg ${results.exoplanetDetected ? 'bg-green-500/10 border-green-500/20' : 'bg-yellow-500/10 border-yellow-500/20'} border`}>
                    <div className="flex items-center space-x-3">
                      {results.exoplanetDetected ? <CheckCircle className="w-8 h-8 text-green-500" /> : <AlertCircle className="w-8 h-8 text-yellow-500" />}
                      <div>
                        <p className="font-heading font-bold text-foreground text-lg">{results.exoplanetDetected ? 'Potential Exoplanet Candidate!' : 'No Clear Transit Signal'}</p>
                        <p className="font-body text-sm text-muted-foreground">Processing Time: {results.processingTime}s</p>
                      </div>
                    </div>
                  </div>
                  
                  {/* NEW VISUALS COMPONENT */}
          ...
          <CosmicCard className="max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="p-6 flex-grow overflow-auto">
                <LightCurveChart 
                  data={chartData}
                  onExport={() => {
                    const csvContent = "time,flux,is_transit\n" + chartData.map(d => `${d.time},${d.flux},${d.transit ? 1 : 0}`).join('\n');
                    const blob = new Blob([csvContent], { type: 'text/csv' });
                    const url = URL.createObjectURL(blob);
                    const link = document.createElement('a');
                    link.href = url;
                    link.download = 'exovision-processed-data.csv';
                    link.click();
                  }}
                />
              </div>
